...
import { useAdminRegenerateVideo, useAdminRetryVideo, useAdminDeleteVideo } from "@/hooks/useAdmin";
...
export function AdminVideoActionsMenu({ video }: { video: Video }) {
  const [menuOpen, setMenuOpen] = useState(false);
  const [confirmDelete, setConfirmDelete] = useState(false);
  const regenerate = useAdminRegenerateVideo();
  const retry = useAdminRetryVideo();
  ...
  const handleRetry = async () => {
    try {
      ...
      toastSuccess("Retrying", "Resuming from the last completed step.");
    } catch (err) {
  ...
  const handleRegenerate = async () => {
    try {
      ...
      toastSuccess("Regeneration started", "Starting completely over.");
    } catch (err) {
      const message = err instanceof ApiError ? err.message : "Something went wrong. Please try again.";
    ...
    <>
      ...
      {confirmDelete && (
        <ConfirmModal
          ...
          message={`This permanently removes "${video.title}" for ${video.ownerEmail ?? "this user"}. This can't be undone.`}
          confirmLabel="Delete"
          danger
